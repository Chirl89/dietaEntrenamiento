...
// Generate / Retrieve history entries for a given number of days
export function getHistoricalData(profileId, daysCount = 7) {
  ...
  for (let i = daysCount - 1; i >= 0; i--) {
    ...
    if (i === 0) {
      ...
      // Baseline generation for demo / smooth initial rendering
...
export function calculateBadges(profileId) {
  ...
  const maxSteps7d = data7d.reduce((max, d) => Math.max(max, d.steps || 0), 0);
